...
using YesSql;
using static OrchardCore.SongServices.GraphQL.Services.Dealer.DealerPartTypeBuilder;
...
public class DealerPartGraphQLFilter : IGraphQLFilter<ContentItem>
{
    private readonly IClock _clock;

    public DealerPartGraphQLFilter(IClock clock) => _clock = clock;
    ...
    public Task<IQuery<ContentItem>> PreQueryAsync(IQuery<ContentItem> query, IResolveFieldContext context)
    {
        ...
        {
            var DealerQuery = query.With<ContentItemIndex>(c => c.Published && c.Latest).With<DealerPartIndex>(index => index.Name == valueName.Value.ToString()).Take(10000);
            return Task.FromResult(DealerQuery);
        }
