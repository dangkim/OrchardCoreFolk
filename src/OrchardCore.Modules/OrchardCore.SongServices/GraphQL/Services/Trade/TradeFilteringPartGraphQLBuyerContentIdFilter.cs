...
public class TradeFilteringPartGraphQLBuyerContentIdFilter : IGraphQLFilter<ContentItem>
{
    public Task<IQuery<ContentItem>> PreQueryAsync(IQuery<ContentItem> query, IResolveFieldContext context)
    {
        ...
        var listArgs = context.Arguments.Where(argument => argument.Value.Value != null && argument.Key.StartsWith(BuyerContentIdFilter, StringComparison.Ordinal));
        ...
        {
            var offerQuery = query.With<TradeFilteringPartIndex>(index => index.BuyerContentId != String.Empty);
            ...
            foreach (var item in listArgs)
            {
                ...
                {
                    if (comparasion == BuyerContentIdFilter) comparasion = BuyerContentIdFilter + "_eq";
                    var comparasionType = comparasion[^2..]; // The name operator like gt, le, etc.

                    if (comparasionType == "ne")
                    {
                        offerQuery = query.With<TradeFilteringPartIndex>(index => index.BuyerContentId != (string)valueComparasion.Value);
                    }
                    else
                    {
                        offerQuery = query.With<TradeFilteringPartIndex>(index => index.BuyerContentId == (string)valueComparasion.Value);
