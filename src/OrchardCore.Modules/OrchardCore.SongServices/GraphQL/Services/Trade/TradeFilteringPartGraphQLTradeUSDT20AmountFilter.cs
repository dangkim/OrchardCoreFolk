...
public class TradeFilteringPartGraphQLTradeUSDT20AmountFilter : IGraphQLFilter<ContentItem>
{
    public Task<IQuery<ContentItem>> PreQueryAsync(IQuery<ContentItem> query, IResolveFieldContext context)
    {
        ...
        var listArgs = context.Arguments.Where(argument => argument.Value.Value != null && argument.Key.StartsWith(TradeUSDT20AmountFilter, StringComparison.Ordinal));
        ...
        {
            var tradeQuery = query.With<TradeFilteringPartIndex>(index => index.TradeUSDT20Amount > 0);
            ...
            foreach (var item in listArgs)
            {
                ...
                {
                    if (comparasion == TradeUSDT20AmountFilter) comparasion = TradeUSDT20AmountFilter + "_eq";
                    var comparasionType = comparasion[^2..]; // The name operator like gt, le, etc.                                      

                    if (comparasionType == "ge")
                    {
                        tradeQuery = query.With<TradeFilteringPartIndex>(index => index.TradeUSDT20Amount >= (decimal)valueComparasion.Value);
                    }
