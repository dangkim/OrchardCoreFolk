...
public class TradeFilteringPartGraphQLCurrencyFilter : IGraphQLFilter<ContentItem>
{
    public Task<IQuery<ContentItem>> PreQueryAsync(IQuery<ContentItem> query, IResolveFieldContext context)
    {
        ...
        var listArgs = context.Arguments.Where(argument => argument.Value.Value != null && argument.Key.StartsWith(CurrencyOfTradeFilter, StringComparison.Ordinal));
        ...
        {
            var tradeQuery = query.With<TradeFilteringPartIndex>(index => index.CurrencyOfTrade != String.Empty);
            ...
            foreach (var item in listArgs)
            {
                ...
                {
                    if (comparasion == CurrencyOfTradeFilter) comparasion = CurrencyOfTradeFilter + "_eq";
                    var comparasionType = comparasion[^2..]; // The name operator like gt, le, etc.

                    if (comparasionType == "ne")
                    {
                        tradeQuery = query.With<TradeFilteringPartIndex>(index => index.CurrencyOfTrade != (string)valueComparasion.Value);
                    }
                    else
                    {
                        tradeQuery = query.With<TradeFilteringPartIndex>(index => index.CurrencyOfTrade == (string)valueComparasion.Value);
