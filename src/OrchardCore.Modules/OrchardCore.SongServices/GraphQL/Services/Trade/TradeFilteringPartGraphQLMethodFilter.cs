...
public class TradeFilteringPartGraphQLMethodFilter : IGraphQLFilter<ContentItem>
{
    public Task<IQuery<ContentItem>> PreQueryAsync(IQuery<ContentItem> query, IResolveFieldContext context)
    {
        ...
        {
            var tradeQuery = query.With<TradeFilteringPartIndex>(index => index.PaymentMethod != String.Empty);
            ...
            foreach (var item in listArgs)
            {
                ...
                if (comparasion != null && valueComparasion.Value != null)
                {
                    ...
                    {
                        tradeQuery = query.With<TradeFilteringPartIndex>(index => index.PaymentMethod != (string)valueComparasion.Value);
                    }
                    else
                    {
                        tradeQuery = query.With<TradeFilteringPartIndex>(index => index.PaymentMethod == (string)valueComparasion.Value);
