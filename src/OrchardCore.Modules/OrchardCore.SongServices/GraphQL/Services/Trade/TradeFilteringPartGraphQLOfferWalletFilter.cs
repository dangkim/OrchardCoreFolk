...
public class TradeFilteringPartGraphQLOfferWalletFilter : IGraphQLFilter<ContentItem>
{
    public Task<IQuery<ContentItem>> PreQueryAsync(IQuery<ContentItem> query, IResolveFieldContext context)
    {
        ...
        var listArgs = context.Arguments.Where(argument => argument.Value.Value != null && argument.Key.StartsWith(OfferWalletFilter, StringComparison.Ordinal));
        ...
        {
                var tradeQuery = query.With<TradeFilteringPartIndex>(index => index.OfferWallet != String.Empty);
            ...
            foreach (var item in listArgs)
            {
                ...
                {
                    if (comparasion == OfferWalletFilter) comparasion = OfferWalletFilter + "_eq";
                    var comparasionType = comparasion[^2..]; // The name operator like gt, le, etc.

                    if (comparasionType == "ne")
                    {
                        tradeQuery = query.With<TradeFilteringPartIndex>(index => index.OfferWallet != (string)valueComparasion.Value);
                    }
                    else
                    {
                        tradeQuery = query.With<TradeFilteringPartIndex>(index => index.OfferWallet == (string)valueComparasion.Value);
