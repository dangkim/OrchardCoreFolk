...
public class TradeFilteringPartGraphQLFeeTypeFilter : IGraphQLFilter<ContentItem>
{
    public Task<IQuery<ContentItem>> PreQueryAsync(IQuery<ContentItem> query, IResolveFieldContext context)
    {
        ...
        var listArgs = context.Arguments.Where(argument => argument.Value.Value != null && argument.Key.StartsWith(FeeTypeFilter, StringComparison.Ordinal));
        ...
        {
            var tradeQuery = query.With<TradeFilteringPartIndex>(index => index.FeeType > 0);
            ...
            foreach (var item in listArgs)
            {
                ...
                {
                    if (comparasion == FeeTypeFilter) comparasion = FeeTypeFilter + "_eq";
                    var comparasionType = comparasion[^2..]; // The name operator like gt, le, etc.                                      

                    if (comparasionType == "ge")
                    {
                        tradeQuery = query.With<TradeFilteringPartIndex>(index => index.FeeType >= (decimal)valueComparasion.Value);
                    }
