...
public class TradeFilteringPartGraphQLOfferTypeFilter : IGraphQLFilter<ContentItem>
{
    public Task<IQuery<ContentItem>> PreQueryAsync(IQuery<ContentItem> query, IResolveFieldContext context)
    {
        ...
        {
            var tradeQuery = query.With<TradeFilteringPartIndex>(index => index.OfferType != String.Empty);
            ...
            foreach (var item in listArgs)
            {
                ...
                if (comparasion != null && valueComparasion.Value != null)
                {
                    ...
                    {
                        tradeQuery = query.With<TradeFilteringPartIndex>(index => index.OfferType != (string)valueComparasion.Value);
                    }
                    else
                    {
                        tradeQuery = query.With<TradeFilteringPartIndex>(index => index.OfferType == (string)valueComparasion.Value);
