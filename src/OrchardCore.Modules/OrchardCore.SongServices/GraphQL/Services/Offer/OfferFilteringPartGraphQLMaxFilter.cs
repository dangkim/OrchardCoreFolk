...
public class OfferFilteringPartGraphQLMaxFilter : IGraphQLFilter<ContentItem>
{
    public Task<IQuery<ContentItem>> PreQueryAsync(IQuery<ContentItem> query, IResolveFieldContext context)
    {
        ...
        var listArgs = context.Arguments.Where(argument => argument.Value.Value != null && argument.Key.StartsWith(MaxAmountFilter, StringComparison.Ordinal));
        ...
        {
            var offerQuery = query.With<OfferFilteringPartIndex>(index => index.MaxAmount > 0);
            ...
            foreach (var item in listArgs)
            {
                ...
                {
                    if (comparasion == MaxAmountFilter) comparasion = MaxAmountFilter + "_eq";
                    var comparasionType = comparasion[^2..]; // The name operator like gt, le, etc.

                    if (comparasionType == "ge")
                    {
                        offerQuery = query.With<OfferFilteringPartIndex>(index => index.MaxAmount >= (decimal)valueComparasion.Value);
                    }
