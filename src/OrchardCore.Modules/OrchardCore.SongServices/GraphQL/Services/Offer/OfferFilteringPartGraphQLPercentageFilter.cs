...
public class OfferFilteringPartGraphQLPercentageFilter : IGraphQLFilter<ContentItem>
{
    public Task<IQuery<ContentItem>> PreQueryAsync(IQuery<ContentItem> query, IResolveFieldContext context)
    {
        ...
        var listArgs = context.Arguments.Where(argument => argument.Value.Value != null && argument.Key.StartsWith(PercentageFilter, StringComparison.Ordinal));
        ...
        {
            var offerQuery = query.With<OfferFilteringPartIndex>(index => index.Percentage > 0);
            ...
            foreach (var item in listArgs)
            {
                ...
                {
                    if (comparasion == PercentageFilter) comparasion = PercentageFilter + "_eq";
                    var comparasionType = comparasion[^2..]; // The name operator like gt, le, etc.                                      

                    if (comparasionType == "ge")
                    {
                        offerQuery = query.With<OfferFilteringPartIndex>(index => index.Percentage >= (decimal)valueComparasion.Value);
                    }
