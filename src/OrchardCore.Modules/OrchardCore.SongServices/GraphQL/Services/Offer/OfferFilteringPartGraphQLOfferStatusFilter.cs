...
public class OfferFilteringPartGraphQLOfferStatusFilter : IGraphQLFilter<ContentItem>
{
    public Task<IQuery<ContentItem>> PreQueryAsync(IQuery<ContentItem> query, IResolveFieldContext context)
    {
        ...
        var listArgs = context.Arguments.Where(argument => argument.Value.Value != null && argument.Key.StartsWith(OfferStatusFilter, StringComparison.Ordinal));
        ...
        {
            var offerQuery = query.With<OfferFilteringPartIndex>(index => index.Status != String.Empty);
            ...
            foreach (var item in listArgs)
            {
                ...
                {
                    if (comparasion == OfferStatusFilter) comparasion = OfferStatusFilter + "_eq";
                    var comparasionType = comparasion[^2..]; // The name operator like gt, le, etc.

                    if (comparasionType == "ne")
                    {
                        offerQuery = query.With<OfferFilteringPartIndex>(index => index.Status != (string)valueComparasion.Value);
                    }
                    else
                    {
                        offerQuery = query.With<OfferFilteringPartIndex>(index => index.Status == (string)valueComparasion.Value);
