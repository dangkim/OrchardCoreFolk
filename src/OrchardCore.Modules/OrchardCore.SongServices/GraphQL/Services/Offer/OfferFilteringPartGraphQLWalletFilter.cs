...
public class OfferFilteringPartGraphQLWalletFilter : IGraphQLFilter<ContentItem>
{
    public Task<IQuery<ContentItem>> PreQueryAsync(IQuery<ContentItem> query, IResolveFieldContext context)
    {
        ...
        var listArgs = context.Arguments.Where(argument => argument.Value.Value != null && argument.Key.StartsWith(WalletFilter, StringComparison.Ordinal));
        ...
        {
                var offerQuery = query.With<OfferFilteringPartIndex>(index => index.Wallet != String.Empty);
            ...
            foreach (var item in listArgs)
            {
                ...
                {
                    if (comparasion == WalletFilter) comparasion = WalletFilter + "_eq";
                    var comparasionType = comparasion[^2..]; // The name operator like gt, le, etc.

                    if (comparasionType == "ne")
                    {
                        offerQuery = query.With<OfferFilteringPartIndex>(index => index.Wallet != (string)valueComparasion.Value);
                    }
                    else
                    {
                        offerQuery = query.With<OfferFilteringPartIndex>(index => index.Wallet == (string)valueComparasion.Value);
