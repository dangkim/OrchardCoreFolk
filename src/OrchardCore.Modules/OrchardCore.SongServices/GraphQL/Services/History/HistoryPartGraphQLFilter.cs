...
using YesSql;
using static OrchardCore.SongServices.GraphQL.Services.History.HistoryPartTypeBuilder;
...
public class HistoryPartGraphQLFilter : IGraphQLFilter<ContentItem>
{
    private readonly IClock _clock;

    public HistoryPartGraphQLFilter(IClock clock) => _clock = clock;
    ...
    public Task<IQuery<ContentItem>> PreQueryAsync(IQuery<ContentItem> query, IResolveFieldContext context)
    {
        ...
        var (result, valueResult) = context.Arguments.FirstOrDefault(
            argument => argument.Key.StartsWith(ResultFilter, StringComparison.Ordinal));
        ...
        {
            var HistoryQuery = query.With<ContentItemIndex>(c => c.Published && c.Latest).With<HistoryPartIndex>(index => index.Status == valueStatus.Value.ToString()).Take(10000);
            return Task.FromResult(HistoryQuery);
        }
