...
public class LocationPartGraphQLFloorFilter : IGraphQLFilter<ContentItem>
{
    public Task<IQuery<ContentItem>> PreQueryAsync(IQuery<ContentItem> query, IResolveFieldContext context)
    {
        ...
        var listArgs = context.Arguments.Where(argument => argument.Value.Value != null && argument.Key.StartsWith(FloorFilter, StringComparison.Ordinal));
        ...
        {
            var locationQuery = query.With<LocationPartIndex>(index => index.Floor != String.Empty);
            ...
            foreach (var item in listArgs)
            {
                ...
                {
                    if (comparasion == FloorFilter) comparasion = FloorFilter + "_eq";
                    var comparasionType = comparasion[^2..]; // The name operator like gt, le, etc.

                    if (comparasionType == "ne")
                    {
                        locationQuery = query.With<LocationPartIndex>(index => index.Floor != (string)valueComparasion.Value);
                    }
                    else
                    {
                        locationQuery = query.With<LocationPartIndex>(index => index.Floor == (string)valueComparasion.Value);
