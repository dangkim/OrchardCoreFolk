...
public class LocationPartGraphQLStreetFilter : IGraphQLFilter<ContentItem>
{
    public Task<IQuery<ContentItem>> PreQueryAsync(IQuery<ContentItem> query, IResolveFieldContext context)
    {
        ...
        var listArgs = context.Arguments.Where(argument => argument.Value.Value != null && argument.Key.StartsWith(StreetFilter, StringComparison.Ordinal));
        ...
        {
            var locationQuery = query.With<LocationPartIndex>(index => index.Street != String.Empty);
            ...
            foreach (var item in listArgs)
            {
                ...
                {
                    if (comparasion == StreetFilter) comparasion = StreetFilter + "_eq";
                    var comparasionType = comparasion[^2..]; // The name operator like gt, le, etc.

                    if (comparasionType == "ne")
                    {
                        locationQuery = query.With<LocationPartIndex>(index => index.Street != (string)valueComparasion.Value);
                    }
                    else
                    {
                        locationQuery = query.With<LocationPartIndex>(index => index.Street == (string)valueComparasion.Value);
