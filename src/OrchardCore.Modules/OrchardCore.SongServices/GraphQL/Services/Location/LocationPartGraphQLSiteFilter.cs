...
public class LocationPartGraphQLSiteFilter : IGraphQLFilter<ContentItem>
{
    public Task<IQuery<ContentItem>> PreQueryAsync(IQuery<ContentItem> query, IResolveFieldContext context)
    {
        ...
        var listArgs = context.Arguments.Where(argument => argument.Value.Value != null && argument.Key.StartsWith(SiteFilter, StringComparison.Ordinal));
        ...
        {
            var locationQuery = query.With<LocationPartIndex>(index => index.Site != String.Empty);
            ...
            foreach (var item in listArgs)
            {
                ...
                {
                    if (comparasion == SiteFilter) comparasion = SiteFilter + "_eq";
                    var comparasionType = comparasion[^2..]; // The name operator like gt, le, etc.

                    if (comparasionType == "ne")
                    {
                        locationQuery = query.With<LocationPartIndex>(index => index.Site != (string)valueComparasion.Value);
                    }
                    else
                    {
                        locationQuery = query.With<LocationPartIndex>(index => index.Site == (string)valueComparasion.Value);
