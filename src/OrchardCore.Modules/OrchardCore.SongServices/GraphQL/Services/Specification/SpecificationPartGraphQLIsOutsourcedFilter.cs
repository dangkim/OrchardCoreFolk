...
public class SpecificationPartGraphQLIsOutsourcedFilter : IGraphQLFilter<ContentItem>
{
    public Task<IQuery<ContentItem>> PreQueryAsync(IQuery<ContentItem> query, IResolveFieldContext context)
    {
        ...
        var listArgs = context.Arguments.Where(argument => argument.Value.Value != null && argument.Key.StartsWith(IsOutsourcedFilter, StringComparison.Ordinal));
        ...
        else
        {
            ...
            {
                var specificationQuery = query.With<SpecificationPartIndex>(index => index.IsOutsourced == (bool)valueComparasion.Value);
