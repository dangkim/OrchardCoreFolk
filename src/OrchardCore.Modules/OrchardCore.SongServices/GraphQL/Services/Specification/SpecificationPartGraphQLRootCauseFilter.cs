...
public class SpecificationPartGraphQLRootCauseFilter : IGraphQLFilter<ContentItem>
{
    public Task<IQuery<ContentItem>> PreQueryAsync(IQuery<ContentItem> query, IResolveFieldContext context)
    {
        ...
        var listArgs = context.Arguments.Where(argument => argument.Value.Value != null && argument.Key.StartsWith(RootCauseFilter, StringComparison.Ordinal));
        ...
        {
            var SpecificationQuery = query.With<SpecificationPartIndex>(index => index.RootCause != String.Empty);
            ...
            foreach (var item in listArgs)
            {
                ...
                {
                    if (comparasion == RootCauseFilter) comparasion = RootCauseFilter + "_eq";
                    var comparasionType = comparasion[^2..]; // The name operator like gt, le, etc.

                    if (comparasionType == "ne")
                    {
                        SpecificationQuery = query.With<SpecificationPartIndex>(index => index.RootCause != (string)valueComparasion.Value);
                    }
                    else
                    {
                        SpecificationQuery = query.With<SpecificationPartIndex>(index => index.RootCause == (string)valueComparasion.Value);
