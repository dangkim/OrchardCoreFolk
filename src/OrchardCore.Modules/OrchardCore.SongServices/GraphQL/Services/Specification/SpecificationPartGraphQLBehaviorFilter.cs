...
public class SpecificationPartGraphQLBehaviorFilter : IGraphQLFilter<ContentItem>
{
    public Task<IQuery<ContentItem>> PreQueryAsync(IQuery<ContentItem> query, IResolveFieldContext context)
    {
        ...
        var listArgs = context.Arguments.Where(argument => argument.Value.Value != null && argument.Key.StartsWith(BehaviorFilter, StringComparison.Ordinal));
        ...
        {
            var SpecificationQuery = query.With<SpecificationPartIndex>(index => index.Behavior != String.Empty);
            ...
            foreach (var item in listArgs)
            {
                ...
                {
                    if (comparasion == BehaviorFilter) comparasion = BehaviorFilter + "_eq";
                    var comparasionType = comparasion[^2..]; // The name operator like gt, le, etc.

                    if (comparasionType == "ne")
                    {
                        SpecificationQuery = query.With<SpecificationPartIndex>(index => index.Behavior != (string)valueComparasion.Value);
                    }
                    else
                    {
                        SpecificationQuery = query.With<SpecificationPartIndex>(index => index.Behavior == (string)valueComparasion.Value);
