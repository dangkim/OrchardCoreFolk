...
public class SpecificationPartGraphQLIsIncidentFilter : IGraphQLFilter<ContentItem>
{
    public Task<IQuery<ContentItem>> PreQueryAsync(IQuery<ContentItem> query, IResolveFieldContext context)
    {
        ...
        var listArgs = context.Arguments.Where(argument => argument.Value.Value != null && argument.Key.StartsWith(IsIncidentFilter, StringComparison.Ordinal));
        ...
        else
        {
            ...
            {
                var specificationQuery = query.With<SpecificationPartIndex>(index => index.IsIncident == (bool)valueComparasion.Value);
