...
public class SpecificationPartGraphQLOthersFilter : IGraphQLFilter<ContentItem>
{
    public Task<IQuery<ContentItem>> PreQueryAsync(IQuery<ContentItem> query, IResolveFieldContext context)
    {
        ...
        var listArgs = context.Arguments.Where(argument => argument.Value.Value != null && argument.Key.StartsWith(OthersFilter, StringComparison.Ordinal));
        ...
        {
            var SpecificationQuery = query.With<SpecificationPartIndex>(index => index.Others != String.Empty);
            ...
            foreach (var item in listArgs)
            {
                ...
                {
                    if (comparasion == OthersFilter) comparasion = OthersFilter + "_eq";
                    var comparasionType = comparasion[^2..]; // The name operator like gt, le, etc.

                    if (comparasionType == "ne")
                    {
                        SpecificationQuery = query.With<SpecificationPartIndex>(index => index.Others != (string)valueComparasion.Value);
                    }
                    else
                    {
                        SpecificationQuery = query.With<SpecificationPartIndex>(index => index.Others == (string)valueComparasion.Value);
