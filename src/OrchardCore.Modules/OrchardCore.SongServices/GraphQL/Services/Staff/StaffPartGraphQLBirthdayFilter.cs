...
public class StaffPartGraphQLBirthDayFilter : IGraphQLFilter<ContentItem>
{
    public Task<IQuery<ContentItem>> PreQueryAsync(IQuery<ContentItem> query, IResolveFieldContext context)
    {
        ...
        var listArgs = context.Arguments.Where(argument => argument.Value.Value != null && argument.Key.StartsWith(BirthdayFilter, StringComparison.Ordinal));
        ...
        {
            var locationQuery = query.With<StaffPartIndex>(index => index.Birthday != DateTime.MinValue);
            ...
            foreach (var item in listArgs)
            {
                ...
                {
                    if (comparasion == AvatarIdFilter) comparasion = AvatarIdFilter + "_eq";
                    var comparasionType = comparasion[^2..]; // The name operator like gt, le, etc.

                    if (comparasionType == "ne")
                    {
                        locationQuery = query.With<StaffPartIndex>(index => index.Birthday != (DateTime)valueComparasion.Value);
                    }
                    else
                    {
                        locationQuery = query.With<StaffPartIndex>(index => index.Birthday == (DateTime)valueComparasion.Value);
