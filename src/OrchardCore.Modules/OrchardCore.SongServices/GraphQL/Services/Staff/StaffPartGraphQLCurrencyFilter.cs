...
public class StaffPartGraphQLCurrencyFilter : IGraphQLFilter<ContentItem>
{
    public Task<IQuery<ContentItem>> PreQueryAsync(IQuery<ContentItem> query, IResolveFieldContext context)
    {
        ...
        var listArgs = context.Arguments.Where(argument => argument.Value.Value != null && argument.Key.StartsWith(CurrencyFilter, StringComparison.Ordinal));
        ...
        {
            var staffQuery = query.With<StaffPartIndex>(index => index.Currency != String.Empty);
            ...
            foreach (var item in listArgs)
            {
                ...
                {
                    if (comparasion == CurrencyFilter) comparasion = CurrencyFilter + "_eq";
                    var comparasionType = comparasion[^2..]; // The name operator like gt, le, etc.

                    if (comparasionType == "ne")
                    {
                        staffQuery = query.With<StaffPartIndex>(index => index.Currency != (string)valueComparasion.Value);
                    }
                    else
                    {
                        staffQuery = query.With<StaffPartIndex>(index => index.Currency == (string)valueComparasion.Value);
