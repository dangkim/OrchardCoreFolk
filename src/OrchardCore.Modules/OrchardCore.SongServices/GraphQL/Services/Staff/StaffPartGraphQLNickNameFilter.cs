...
public class StaffPartGraphQLNickNameFilter : IGraphQLFilter<ContentItem>
{
    public Task<IQuery<ContentItem>> PreQueryAsync(IQuery<ContentItem> query, IResolveFieldContext context)
    {
        ...
        var listArgs = context.Arguments.Where(argument => argument.Value.Value != null && argument.Key.StartsWith(NickNameFilter, StringComparison.Ordinal));
        ...
        {
            var StaffQuery = query.With<StaffPartIndex>(index => index.Nickname != String.Empty);
            ...
            foreach (var item in listArgs)
            {
                ...
                {
                    if (comparasion == NickNameFilter) comparasion = NickNameFilter + "_eq";
                    var comparasionType = comparasion[^2..]; // The name operator like gt, le, etc.

                    if (comparasionType == "ne")
                    {
                        StaffQuery = query.With<StaffPartIndex>(index => index.Nickname != (string)valueComparasion.Value);
                    }
                    else
                    {
                        StaffQuery = query.With<StaffPartIndex>(index => index.Nickname == (string)valueComparasion.Value);
