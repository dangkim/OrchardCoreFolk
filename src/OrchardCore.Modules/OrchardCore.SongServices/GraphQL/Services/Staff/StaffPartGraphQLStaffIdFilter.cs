...
public class StaffPartGraphQLStaffIdFilter : IGraphQLFilter<ContentItem>
{
    public Task<IQuery<ContentItem>> PreQueryAsync(IQuery<ContentItem> query, IResolveFieldContext context)
    {
        ...
        var listArgs = context.Arguments.Where(argument => argument.Value.Value != null && argument.Key.StartsWith(StaffIdFilter, StringComparison.Ordinal));
        ...
        {
            var staffQuery = query.With<StaffPartIndex>(index => index.StaffId != String.Empty);
            ...
            foreach (var item in listArgs)
            {
                ...
                {
                    if (comparasion == StaffIdFilter) comparasion = StaffIdFilter + "_eq";
                    var comparasionType = comparasion[^2..]; // The name operator like gt, le, etc.

                    if (comparasionType == "ne")
                    {
                        staffQuery = query.With<StaffPartIndex>(index => index.StaffId != (string)valueComparasion.Value);
                    }
                    else
                    {
                        staffQuery = query.With<StaffPartIndex>(index => index.StaffId == (string)valueComparasion.Value);
