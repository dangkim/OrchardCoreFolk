...
public class StaffPartGraphQLAvatarIdFilter : IGraphQLFilter<ContentItem>
{
    public Task<IQuery<ContentItem>> PreQueryAsync(IQuery<ContentItem> query, IResolveFieldContext context)
    {
        ...
        var listArgs = context.Arguments.Where(argument => argument.Value.Value != null && argument.Key.StartsWith(AvatarIdFilter, StringComparison.Ordinal));
        ...
        {
            var locationQuery = query.With<StaffPartIndex>(index => index.AvatarId != String.Empty);
            ...
            foreach (var item in listArgs)
            {
                ...
                if (comparasion != null && valueComparasion.Value != null)
                {
                    ...
                    {
                        locationQuery = query.With<StaffPartIndex>(index => index.AvatarId != (string)valueComparasion.Value);
                    }
                    else
                    {
                        locationQuery = query.With<StaffPartIndex>(index => index.AvatarId == (string)valueComparasion.Value);
