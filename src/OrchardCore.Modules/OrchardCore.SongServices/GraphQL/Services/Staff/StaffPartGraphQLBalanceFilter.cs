...
public class StaffPartGraphQLBalanceFilter : IGraphQLFilter<ContentItem>
{
    public Task<IQuery<ContentItem>> PreQueryAsync(IQuery<ContentItem> query, IResolveFieldContext context)
    {
        ...
        var listArgs = context.Arguments.Where(argument => argument.Value.Value != null && argument.Key.StartsWith(BalanceFilter, StringComparison.Ordinal));
        ...
        {
            var StaffQuery = query.With<StaffPartIndex>(index => index.Balance != 0);
            ...
            foreach (var item in listArgs)
            {
                ...
                {
                    if (comparasion == BalanceFilter) comparasion = BalanceFilter + "_eq";
                    var comparasionType = comparasion[^2..]; // The name operator like gt, le, etc.

                    if (comparasionType == "ne")
                    {
                        StaffQuery = query.With<StaffPartIndex>(index => index.Balance != (decimal)valueComparasion.Value);
                    }
                    else
                    {
                        StaffQuery = query.With<StaffPartIndex>(index => index.Balance == (decimal)valueComparasion.Value);
