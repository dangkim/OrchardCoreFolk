...
namespace OrchardCore.SimService.SimApi
{
    ...
    [OpenApiTag("Order management", Description = "Get information of order of Ware House Three.")]
    public class OrderWareHouseThreeProfileController : Controller
    {
        ...
        public OrderWareHouseThreeProfileController(
        ...
        #region Check order
        /// <summary>
        /// Check order of Ware House Three (Get SMS)
        /// </summary>
        /// <remarks>
        /// 
        /// </remarks> 
        [HttpGet]
        [ActionName("checkOrderWareHouseThree")]
        [ProducesResponseType(typeof(CheckOrderWareHouseThreeDto), (int)HttpStatusCode.OK)]
        ...
        [ReDocCodeSample("php", "$token = 'Your token';" +
            "\n$ch = curl_init();" +
            "\n$id = 1;" +
            "\ncurl_setopt($ch, CURLOPT_URL, 'https://openapi.simforrent.com/api/content/checkOrderWareHouseThree/'. $id);" +
        ...
        [ReDocCodeSample("python", "import requests" +
            ...
            "\n}" +
            "\nresponse = requests.get('https://openapi.simforrent.com/api/content/checkOrderWareHouseThree/' + id, headers=headers)")]
        [ReDocCodeSample("c#", "var client = new RestClient(\"https://openapi.simforrent.com/api/content/checkOrderWareHouseThree\");" +
            "\nvar request = new RestRequest();" +
            "\nrequest.AddHeader(\"Authorization\", \"Bearer \" + token);" +
            "\nrequest.AddHeader(\"Accept\", \"application/json\");" +
            "\nvar response = await client.ExecuteGetAsync(request);")]
        public async Task<ActionResult<CheckOrderWareHouseThreeDto>> CheckOrderWareHouseThreeAsync(string id, int inventory = (int)InventoryEnum.CSim)
        {
            ...
            var resObject = await ApiCommon.CheckOrderWareHouseThreeAsync(cSimToken, id, inventory);
            ...
            resObject.Phone = orderDetailPart.Phone;
        ...
        #region Cancel order
        /// <summary>
        /// Cancel order of Ware House Three
        /// </summary>
        /// <remarks>
        /// 
        /// </remarks> 
        [HttpGet]
        [ActionName("cancelOrderWareHouseThree")]
        ...
        [ReDocCodeSample("php", "$token = 'Your token';" +
            "\n$ch = curl_init();" +
            "\n$id = 1;" +
            "\ncurl_setopt($ch, CURLOPT_URL, 'https://openapi.simforrent.com/api/user/cancelOrderWareHouseThree/'. $id);" +
            "\ncurl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);" +
            "\ncurl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');" +
            "\n$headers = array();" +
            "\n$headers[] = 'Authorization: Bearer ' . $token;" +
        ...
        [ReDocCodeSample("python", "import requests" +
            ...
            "\n'Accept': 'application/json'," +
            "\n}" +
            "\nresponse = requests.get('https://openapi.simforrent.com/api/user/cancelOrderWareHouseThree/' + id, headers=headers)")]
        [ReDocCodeSample("c#", "var client = new RestClient(\"https://openapi.simforrent.com/api/user/cancelOrderWareHouseThree\");" +
            "\nvar request = new RestRequest();" +
            "\nrequest.AddHeader(\"Authorization\", \"Bearer \" + token);" +
            "\nrequest.AddHeader(\"Accept\", \"application/json\");" +
            "\nvar response = await client.ExecuteGetAsync(request);")]
        public async Task<ActionResult<OrderDetailPartViewModel>> CancelOrderWareHouseThreeAsync(string id)
        {
            var user = await _userManager.GetUserAsync(User) as Users.Models.User;
            if (user == null || !user.IsEnabled || !user.EmailConfirmed) return BadRequest();
            if (!await _authorizationService.AuthorizeAsync(User, SimApiPermissions.AccessContentApi))
            ...
            id = id.Remove(0, 1);
            var url = string.Format("https://chothuesimcode.com/api?act=expired&apik={0}&id={1}", cSimToken, id);

            var client = new RestClient(url);
            var request = new RestRequest();
            request.AddHeader("Authorization", "Bearer " + cSimToken);
