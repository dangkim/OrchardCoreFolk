...
namespace OrchardCore.SimService.Controllers
{
    internal static class ApiControllerExtensions
    {
        ...
        internal static async Task<string> ForgetPasswordUser(this Controller controller, User user, string confirmationEmailSubject, ILogger logger, string redirectUrl)
        {
            if (controller.ModelState.IsValid)
            {
                if (user != null && controller.ModelState.IsValid)
                {
                    logger.LogInformation(3, "User reset a password.");

                    // Send an email with this link
                    var callbackUrl = await controller.SendEmailForgetPasswordTokenAsync(user, confirmationEmailSubject, redirectUrl);
        ...
        internal static async Task<string> SendEmailForgetPasswordTokenAsync(this Controller controller, User user, string subject, string redirectUrl)
        {
            ...
            var callbackUrl = $"{redirectUrl}/resetpassword-email/{user.UserId}/{HttpUtility.UrlEncode(code)}";
